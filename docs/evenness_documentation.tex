...
skipped. The score itself uses \emph{no} labels, but for consistency with the classification
models a \emph{step gate} (\S\,\ref{sec:gate}) removes steps that fall in or across a labelled
transition; where a recording carries no surface/transition markers, every detected step is kept.
...
\section{Step gate (consistency with the classification models)}\label{sec:gate}
The evenness score is unsupervised, but to make it operate on the \emph{same} steps the supervised
models keep, recordings that carry surface/transition markers are passed through the identical step
gate used there. Each surface or \code{transition}/\code{wait} marker opens a row region running to
the next marker; \code{transition}/\code{wait} regions are \emph{excluded}. A detected step is then
\textbf{dropped} if its window (i)~touches an excluded (transition/wait) row, (ii)~straddles two
different labelled surfaces, or (iii)~is less than $80\%$ inside a single labelled surface
(a partial / segment-edge step). Recordings without markers are ungated (all steps kept), so the
score remains fully label-free wherever labels are absent.

\medskip\noindent\emph{Note.} The evenness notebook and the classification notebooks segment steps
by slightly different peak detectors (total pressure vs.\ heel sensors), so after identical gating
the two step \emph{counts} still differ by ${\sim}1\%$; this is a detection difference, not a gating
one, and is immaterial to a distribution-level score fitted over thousands of steps.
...
continuous\_paved & ${\approx}\,55$--$56$  & firm, smooth, regular --- \emph{most even} \\
modular\_paved    & ${\approx}\,50$--$51$  & firm but bumpy (cobblestone) --- below continuous paved \\
\bottomrule
\end{tabular}
\end{center}
...
within the firm surfaces it orders them by \emph{regularity}: continuous paved (smooth) scores
highest, while \textbf{modular paved} (cobblestone / uneven brick) sits below it --- the score picks
up the bumpiness of cobblestone even though both are rigid. It is \textbf{not} a paved-vs-loose
material discriminator (loose compact dirt and pavement score similarly, which is honest --- a smooth
dirt path really is about as regular to walk on as pavement). Evenness measures how steady the
walking is --- the soft-vs-firm and smooth-vs-bumpy axes --- not surface material \emph{per se}.
\emph{Caveat:} the modular\_paved figure rests on limited data (currently one walker's cobblestone
recordings), so its position is indicative, not yet firmly established.
...
grass scores distinctly low, firm surfaces high (and cobblestone below smooth pavement) --- making it a label-free companion to the
