...
\title{\textbf{Feature Documentation — Merged 3-Class Model (v16)}\\[2pt]
\large grass / loose / paved}
...
The model uses \textbf{48 features}. The $F$ values are ANOVA class-separation scores on the full
data (higher $=$ separates grass/loose/paved better). Person-confound features that are computed
but \textbf{dropped} are listed at the end.
...
\end{equation}
\begin{itemize}
  \item \code{orient_rest_a_roll3_std} \Fval{1612} and \code{orient_rest_b_roll3_std} \Fval{889}
...
\end{align}
\begin{itemize}
  \item \code{accel_jerk_ratio} \Fval{1205} --- jerk ratio of the vertical axis $a^y$
  (amplitude-invariant); sharp/jerky strikes on hard ground, damped on compliant.
  \item \code{accel_hf_frac} \Fval{549} --- HF fraction of $\lVert a_t\rVert$; higher on firm surfaces.
  \item \code{accel_spec_centroid} \Fval{517} --- spectral centroid of $\lVert a_t\rVert$; shifts up
  with harder impacts.
  \item \code{accel_spec_entropy} \Fval{492} --- spectral entropy of $\lVert a_t\rVert$ (tonal vs
  broadband texture).
...
\begin{itemize}
  \item \code{gyro_x_std} \Fval{1046} --- standard deviation of the $x$ angular rate over the step,
  ...
  \item \code{gyro_hf_frac} \Fval{397} --- HF fraction (same formula) of $\lVert g_t\rVert$.
...
\begin{itemize}
  \item \code{stance_duration} \Fval{838} --- loaded time per step,
...
\begin{itemize}
  \item \code{heel_fore_ratio} \Fval{372} --- $\bar h/\bar f$ (front-to-back load).
  \item \code{heel_impulse_norm} \Fval{394} --- $\sum_t h_t/\text{scale}$ (time-under-load at heel).
...
\end{align}
\begin{itemize}
  \item \code{gu_spatial_entropy_stance} \Fval{510} and \code{gu_spatial_entropy_ff} --- normalised
...
describe \emph{compliance} (soft vs firm) very well, which is why grass-vs-hard is reliable; they
describe \emph{fine firm-surface texture} weakly, which is why loose-vs-paved remains hard.
